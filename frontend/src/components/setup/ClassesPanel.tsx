 ...
 * Self-contained, matching `TeachersPanel`/`TeachingAssignmentsPage`'s
 * architecture exactly. The canonical WHOLE_CLASS `ParticipantGroup`
 * (Owner Decision #33) is never exposed anywhere in this component --
 * there is no group id/role/membership/ordinal field on `ClassSectionProjectionItem`
 * at all, so there is nothing to accidentally render.
 */

type PanelState =
  | { status: "loading" }
  | { status: "error"; message: string }
  | { status: "ready"; projection: ClassesProjectionResponse };
...
const REFRESH_FAILURE_MESSAGE =
  "The change was saved, but the latest class list could not be refreshed. Retry to load the current data.";
const LOCK_RACE_MESSAGE = "A schedule was generated since this page loaded. Classes are now read-only.";
  ...
  switch (error.code) {
    ...
    case "CLASS_IN_USE": {
      ...
      if (Array.isArray(rawReferencedBy) && rawReferencedBy.length > 0) {
        ...
        return `This class can't be deleted because it is used by: ${labels.join(", ")}.`;
      }
      return error.detail;
    }
    case "INVALID_CLASS": {
...
type EditState = { classId: string; name: string };
...
function ClassesPanel() {
  ...
  useEffect(() => {
    ...
    getClasses(appConfigResult.schoolId, appConfigResult.academicYearId, controller.signal)
      .then((projection) => {
  ...
  const refreshProjection = useCallback(
    async (staleFailureMessage: string) => {
      ...
      try {
        const projection = await getClasses(appConfigResult.schoolId, appConfigResult.academicYearId);
        setPanelState({ status: "ready", projection });
  ...
  const handleSubmitCreate = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      setCreateError(null);
      const body: ClassSectionWriteRequest = { name: createName.trim() };
      const outcome = await performWrite(
        () => createClass(appConfigResult.schoolId, appConfigResult.academicYearId, body),
        setCreateError,
      );
  ...
  const handleStartEdit = useCallback((classSection: ClassSectionProjectionItem) => {
    setEditState({ classId: classSection.id, name: classSection.name });
    setEditError(null);
    setStaleLockNotice(null);
  }, []);
  ...
  const handleSubmitEdit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      setEditError(null);
      const body: ClassSectionWriteRequest = { name: editState.name.trim() };
      const outcome = await performWrite(
        () => updateClass(appConfigResult.schoolId, appConfigResult.academicYearId, editState.classId, body),
        setEditError,
      );
  ...
  const handleDeleteClick = useCallback((classId: string) => {
  ...
  const handleConfirmDelete = useCallback(
    async (classId: string) => {
      ...
      const outcome = await performWrite(
        () => deleteClass(appConfigResult.schoolId, appConfigResult.academicYearId, classId),
        setDeleteError,
      );
  ...
  return (
    <div className="setup-panel">
      {locked && (
        <div className="lock-banner" id="classes-lock-banner-text">
      ...
      {createOpen && (
        <form className="create-panel" onSubmit={handleSubmitCreate}>
          <h3 className="create-panel-heading">Add class</h3>
          <label>
            Class name
            <input
              type="text"
      ...
      {projection.classes.length === 0 ? (
        <div className="setup-empty-state">
          <p>No classes yet.</p>
          <p>Add the classes that will be scheduled.</p>
          {!createOpen && (
            <button type="button" className="btn-primary" onClick={handleOpenCreate} disabled={createDisabled}>
              + Add class
            </button>
        ...
        <div className="table-scroll">
          <table className="setup-table">
            ...
            <tbody>
              {projection.classes.map((classSection) => (
                <tr key={classSection.id}>
                  {editState !== null && editState.classId === classSection.id ? (
                    <>
                      <td>
                    ...
                    <>
                      ...
                      <td data-label="Actions">
                        {deleteConfirmId === classSection.id ? (
                          <span className="delete-confirm">
                            <span className="delete-confirm-text">Delete {classSection.name}?</span>
                            {deleteError !== null && (
                            ...
                            <span className="delete-confirm-actions">
                              <button
                                type="button"
                                onClick={() => handleConfirmDelete(classSection.id)}
                          ...
                          <span className="actions-group">
                            <button type="button" className="action-button" disabled aria-describedby="classes-lock-banner-text">
                              Edit
                            </button>
                            <button
                              type="button"
                              ...
                              disabled
                              aria-describedby="classes-lock-banner-text"
                            >
                              Delete
                            </button>
                          </span>
                          ...
                          <span className="actions-group">
                            <button
                              ...
                              onClick={() => handleStartEdit(classSection)}
                            ...
                            <button
                              type="button"
                              className="action-button action-button-delete"
                              onClick={() => handleDeleteClick(classSection.id)}
                              disabled={controlsDisabled || createOpen || editState !== null || deleteConfirmId !== null}
                            >
                              Delete
                            </button>
